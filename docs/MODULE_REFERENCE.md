...
| `fincli/stock_screening/content/stock_table.py` | Top-level HTML table extractor. Uses BeautifulSoup to locate the screener result table and extract all rows. `has_empty_marker` reports whether the page carries Finviz's `js-screener-body-empty` zero-result marker — the discriminator `fincli.app.main.aggregate_rows` uses to tell a legitimate zero-match screen apart from a missing/malformed table. |
| `fincli/stock_screening/parsers/stock_table.py` | Per-row HTML cell parser. Converts raw `<td>` cell text to typed Python values (strings, floats, ints). Raises `AttributeError` when the link anchor is missing — caught by the Pillar-4 classifier as a DATA-class failure. `ticker_symbol(cells)` extracts the ticker from the last `<a>` in the ticker cell (not the whole cell's text) and validates it against that anchor's href `t` query param, raising `ScreenerLayoutError` on disagreement — fixes GitHub issue #14, where Finviz's redesigned layout's logo-fallback anchor duplicated the ticker's first letter (e.g. "KTOS" -> "KKTOS"). |
| `fincli/stock_screening/locators/stock_table_locators.py` | CSS / element locators for the Finviz screener table — used by both the content extractor and the per-row parser to keep selectors in one place. `EMPTY_BODY_ID` is the id of Finviz's legitimate zero-result marker table. |
| `fincli/stock_screening/errors.py` | `ScreenerLayoutError` — raised when Finviz HTML doesn't match the expected screener layout contract (missing table with no empty-result marker, or a ticker cell whose text disagrees with its href). Classified DATA (exit 4 / HTTP 502 `parsing`) by `fincli.app.exit_codes.classify`. |

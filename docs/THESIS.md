...
| Date | Change |
|------|--------|
| 2026-05-24 | **fincli HTTP API mode shipped.** Added `fincli_api/` sibling package exposing the screener over REST+JSON. 9 commits per `docs/features/fincli-api-plan.md`: skeleton + Pydantic models (3-way parallel) + adapter boundary + routes + classifier-driven exception handler (4-way parallel + fix-loop) + 30-test pyramid (unit + integration + 3 live Finviz smoke tests) + committed OpenAPI 3.1.0 snapshot at `docs/api/openapi.{yaml,json}` for polyglot codegen (oapi-codegen, openapi-generator, etc.). The umbrella validates the "fincli is consumable by a downstream pipeline" claim that pipeline-mode + list-filters established — Go/TS/Rust callers can now hit `POST /screens` with `{filters: {fa_pe: "u5", sec: "energy"}}` and receive typed JSON without touching Python. Design spec: `docs/superpowers/specs/2026-05-22-fincli-api-design.md`. Implementation plan: `docs/features/fincli-api-plan.md`. |

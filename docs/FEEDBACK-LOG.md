...
### 2026-06-27 — Decision: always work in a git worktree, never on the branch directly

**What:** From the next session onward, every change starts in a **dedicated git worktree on its own feature branch** — `git worktree add ../algo_beta-<topic> -b <type>/<topic>` — and all edits, commits, and test runs happen there. Never edit/commit directly on the primary `master` checkout. Branch first, commit scoped to only the touched files, then open a PR. Codified as a standing rule in `CLAUDE.md` → Conventions → **Git Workflow**.

**Why:** User direction (2026-06-27). The primary working tree carries pre-existing untracked artifacts (`.codex/`, `docs/pendingwork/`, `uv.lock`, `workspace_output/`) plus a live editable install; mixing task work into it risks (a) accidentally sweeping unrelated files into a commit (`git add -A` over a dirty tree), (b) one in-flight change contaminating an unrelated run, and (c) committing straight to `master`. A worktree gives each task a clean, isolated checkout while sharing the same object store, so switching tasks costs nothing and `master` stays pristine. This session's launcher-removal work already followed the spirit of it — scoped staging of exactly 12 files on `chore/remove-dead-launchers`, never touching the untracked artifacts — but it ran in the main checkout rather than a worktree; the rule makes the worktree mandatory going forward.

**How to apply:** At the start of any task: `git worktree add ../algo_beta-<topic> -b <type>/<topic>` (branch types: `feat/`, `fix/`, `chore/`, `docs/`, `refactor/`), `cd` into it, `pip install -e ".[dev]"` if its `.venv` isn't shared, then work. When done: commit (scoped) in the worktree, then **STOP and report integration options — do NOT push or open a PR on your own initiative** (see the 2026-06-29 entry below); `git worktree remove` only after the user has chosen an integration path. If a session somehow begins on a bare `master` checkout, create the branch+worktree before the first edit. This is an `algo_beta`-level rule today; it is a candidate to promote to the Strade parent so all delegated tools share it — raise that with the user before editing parent docs (delegation boundary).

---

### 2026-06-29 — Integration is NOT automatic; never push or open a PR on your own initiative

**What:** Finishing work in a worktree does **not** imply integrating it. Default completion = leave the reviewed, green branch in its worktree and **report the integration options** — let the user pick. A **local fast-forward** of `master` is allowed **only when the user explicitly asks**. **Pushing to `origin` and opening a PR are SEPARATE, explicitly user-initiated steps — never do them on your own initiative.** This supersedes the 2026-06-27 entry's "When done: … push, open a PR" finish step, which over-implied auto-merge.

**Why:** honors the global "commit or push only when the user asks" rule; the user has repeatedly declined unsolicited PRs. Auto-advancing `master`/the remote removes the review gate and is hard to reverse across the multi-session/worktree setup. Workspace-wide decision (mirrors the same entry in `midas/docs/FEEDBACK-LOG.md` and `agents/rules/preflight.md` Step 0).

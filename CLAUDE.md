...
- **Layout drift and ticker-cell corruption raise `ScreenerLayoutError`** (`fincli/stock_screening/errors.py`, resolved 2026-08-11, closes MAJOR #4 + GitHub issue #14). A missing screener `<table>` with no `js-screener-body-empty` marker present, or a ticker cell whose visible text disagrees with its link href (e.g. Finviz's redesigned layout duplicating the first letter, "KTOS" -> "KKTOS"), now raises instead of silently returning bad data or coercing to a 200 empty result. Classified DATA (exit 4 / HTTP 502 `parsing`) by `fincli.app.exit_codes.classify`. A genuine zero-match screen still carries the empty-result marker and stays a 200/`exit 0` success.
